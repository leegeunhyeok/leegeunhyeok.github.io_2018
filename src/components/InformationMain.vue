    ...
    <transition name="fade" mode="out-in">
      <div id="information-wrap" v-if="view === 0">
        <div id="info-btn" @click="view = 1">
          ...
          <div>{{ language[$store.state.language].info }}</div>
        </div>
        <div id="changelog-btn" @click="view = 2">
          <img src="../assets/changelog.png">
          <div>{{ language[$store.state.language].changelog }}</div>
        </div>
      </div>
      <info-view v-else-if="view === 1" @onBack="view = 0"></info-view>
      <changelog-view v-else @onBack="view = 0"></changelog-view>
    </transition>
  </div>
</template>
