<template>
  <div id="intro">
    <div class="intro-text-area">
      <div class="intro-text">{{language[lang].portfolio}}</div>
      <p class="intro-sub-text-1">{{language[lang].name}}</p>
    </div>
    <div class="power-btn-area">
      <img src="../assets/power.png" @click="$emit('onPower')">
      <p class="intro-sub-text-2">{{language[lang].power}}</p>
    </div>
    <div class="language-btn-area">
      <div class="language-text">{{changeLang}}</div>
      <transition name="fade" mode="out-in">
        <button class="language-btn" @click="change" v-if="lang === 'en'">Korean</button>
        <button class="language-btn" @click="change" v-else>영어</button>
      </transition>
    </div>  
  </div>
</template>
